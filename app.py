...
if quickbooks_file and commodity_file:
    ...
    selection_mode = st.sidebar.selectbox("Selection Mode", ["Auto-select Best", "Manual Selection", "Select Category for Hedge"])

    if selection_mode == "Select Category for Hedge":
        selected_category = st.sidebar.selectbox("Choose Expense Category", expense_df["Expense_Category"].unique())
        if selected_category:
            specific_correlations = [(commodity, category, corr) for (commodity, category), corr in correlate_expenses_with_commodities(expense_df, commodity_df) if category == selected_category]
            ...
            for idx, rec in enumerate(best_recommendations, start=1):
                ...
                st.write(f"**Potential Savings over Duration:** ${rec['Potential Savings']:.2f}")
                ...
                ax.set_title(f"Monthly Expected Returns for {rec['Commodity']} in {selected_category}")
                ax.set_xlabel("Month")
                ax.set_ylabel("Expected Return (%)")
                st.pyplot(fig)

    elif selection_mode == "Auto-select Best":
